  ...
  return (
    <div style={{minHeight:'100vh',background:`linear-gradient(135deg,${C.navyDark},${C.navy})`,display:'flex',alignItems:'center',justifyContent:'center',padding:'24px',fontFamily:'Calibri,sans-serif'}}>
      <div style={{background:C.white,borderRadius:'20px',padding:'40px 32px',width:'100%',maxWidth:'360px',boxShadow:'0 24px 64px rgba(0,0,0,0.3)',textAlign:'center'}}>
        <h1 style={{fontFamily:'Georgia,serif',fontSize:'20px',color:C.navy,marginBottom:'4px',fontWeight:700}}>Mana Social</h1>
        <p style={{fontSize:'12px',color:C.muted,marginBottom:'28px',letterSpacing:'0.05em',textTransform:'uppercase'}}>P&L Tracker</p>
...
export default function ManaSocialApp() {
  ...
  const handleSave = async () => {
    ...
    if (t === 'sales') {
      ...
      if (!formData.amount || !formData.label) return alert('Missing fields')
      payload = { seller_name: formData.label, total_cost: Number(formData.amount), notes: `Qty: ${formData.itemCount} | ${formData.notes}`, due_date: formData.date, entity: getEntity(formData.date) }
    } else if (t === 'expenses') {
      ...
      let totalUnits = 0
      if (formData.cogsType === 'collection') totalUnits = parseInt(formData.cogsCards)||0
      else if (['booster_box','precon'].includes(formData.cogsType)) totalUnits = (parseInt(formData.cogsCardsPerBox)||0) * qty
      else totalUnits = qty
  ...
  const handleUpdateSold = async (id: string, soldUnits: number) => {
    await supabase.from('cogs_inventory').update({ sold_units: soldUnits }).eq('id', id)
    fetchData()
  }
  ...
  const renderForm = () => {
    ...
    const cogsPreview = () => {
      ...
      let units = formData.cogsType==='collection' ? parseInt(formData.cogsCards)||0 : ['booster_box','precon'].includes(formData.cogsType) ? (parseInt(formData.cogsCardsPerBox)||0)*qty : qty
    ...
    return (
      <div style={{...card,border:`1px solid ${C.teal}`}}>
        ...
        <div style={{display:'grid',gap:'10px'}}>
          ...
          {t==='cogs_inventory' && <>
            <div>
              <span style={lbl}>Inventory Type</span>
    ...
    switch(activeTab) {
      ...
      case 'cogs': return (
        <div>
          ...
          {cogsInventory.map(r=>{
            const sold = r.sold_units||0
            const ratio = r.total_units>0?Math.min(sold/r.total_units,1):0
            const recog = parseFloat(r.total_cost||0)*ratio
            return (
              <div key={r.id} style={card}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:'8px'}}>
                  <div>
                    <div style={{fontWeight:700}}>{r.description}</div>
                    {r.set_name && <div style={{fontSize:'12px',color:C.muted}}>{r.set_name}</div>}
                    <div style={{fontSize:'11px',color:C.muted}}>{r.date} · {r.inventory_type}</div>
                  </div>
                  <div style={{textAlign:'right'}}>
                    <div style={{fontWeight:700,color:C.pink}}>{fmt(parseFloat(r.total_cost||0))}</div>
                    <div style={{fontSize:'11px',color:C.muted}}>${parseFloat(r.cost_per_unit||0).toFixed(3)}/unit</div>
                  </div>
                </div>
                <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'6px'}}>
                  <span style={{fontSize:'12px',color:C.muted}}>Sold:</span>
                  <input type="number" min="0" max={r.total_units} value={sold} onChange={e=>handleUpdateSold(r.id,parseInt(e.target.value)||0)}
                    style={{...inp,width:'80px',padding:'4px 8px',fontSize:'13px',textAlign:'center'}} />
      ...
      case 'deductions': return (
        <div>
          <button onClick={()=>setShowSettings(!showSettings)} style={{...card,width:'100%',textAlign:'left',cursor:'pointer',display:'flex',justifyContent:'space-between',padding:'14px 16px',marginBottom:'10px',border:`1px solid ${C.border}`}}>
            <span style={{fontWeight:700,color:C.navy}}>⚙ Deduction Settings</span>
          ...
          {showSettings && (
            <div style={{...card,borderColor:C.teal}}>
              ...
              <div style={{fontSize:'13px',color:C.text}}>🚗 Mileage: {MILEAGE_RATE*100}¢/mile · {totalMiles.toFixed(1)} miles logged · <strong style={{color:C.teal}}>{fmt(mileageDeduction)}</strong> deduction</div>
            </div>
          )}
          ...
          <div style={card}>
            ...
            {expenses.map(e=>{
              ...
              const ded = rule.pct===100?amt:e.category==='Home Office'?amt*(homeOfficePct/100):0
              return (
      ...
      case 'tax': return (
        <div>
          <div style={{padding:'12px 14px',borderRadius:'10px',background:'rgba(240,192,64,0.1)',border:'1px solid rgba(240,192,64,0.3)',marginBottom:'12px',fontSize:'12px',color:'#7A5A00'}}>
            ⚠️ Estimates only — 22% federal, 9.3% CA. Confirm with Kannie before paying.
          </div>
    ...
    <div style={{fontFamily:'Calibri,sans-serif',background:C.bg,minHeight:'100vh',paddingBottom:'140px'}}>
      <div style={{maxWidth:'500px',margin:'0 auto',padding:'16px'}}>
        <header style={{marginBottom:'16px'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'12px'}}>
            <div>
              <div style={{fontWeight:900,color:C.navy,fontSize:'15px',letterSpacing:'-0.3px'}}>MANA SOCIAL LLC</div>
              <div style={{fontSize:'10px',color:C.teal,letterSpacing:'0.05em',textTransform:'uppercase'}}>P&L Tracker</div>
            </div>
      ...
      <nav style={{position:'fixed',bottom:0,left:0,right:0,background:C.white,borderTop:`2px solid ${C.border}`,display:'flex',overflowX:'auto',zIndex:400,height:'100px'}}>
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>setActiveTab(t.id)} style={{flex:'0 0 auto',minWidth:'60px',border:'none',background:'none',fontSize:'9px',fontWeight:900,color:activeTab===t.id?C.teal:C.muted,padding:'8px 4px',cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:'4px'}}>
