      ...
      <div style={{ display: 'grid', gap: '10px' }}>
        ...
        {t === 'payroll' && <>
          <div><span style={lbl}>Employee</span>
            <select value={formData.label || 'Kiedan'} onChange={e => set({ label: e.target.value })} style={inp}>
              <option value="Kiedan">Kiedan</option><option value="Kayliana">Kayliana</option><option value="Other">Other</option>
            </select>
          </div>
          <div><span style={lbl}>Pay Period</span><input value={formData.payPeriod} onChange={e => set({ payPeriod: e.target.value })} placeholder="e.g. May 1–15 2026" style={inp} /></div>
        ...
        {t === 'mileage_log' && <>
          <div><span style={lbl}>Trip Purpose</span><input value={formData.milePurpose} onChange={e => set({ milePurpose: e.target.value })} placeholder="e.g. Post office run" style={inp} /></div>
          <div><span style={lbl}>Miles</span><input type="number" step="0.01" value={formData.miles} onChange={e => set({ miles: e.target.value })} placeholder="0.00" style={inp} /></div>
          {Number(formData.miles) > 0 && (
            <div style={{ padding: '8px', background: 'rgba(45,191,184,0.08)', borderRadius: '7px', fontSize: '12px', color: C.teal }}>≈ {fmt(Number(formData.miles) * MILEAGE_RATE)} deduction (${MILEAGE_RATE}/mi)</div>
          )}
        </>}

        {t === 'assets' && <>
          <div><span style={lbl}>Category</span>
            <select value={formData.assetCategory} onChange={e => set({ assetCategory: e.target.value })} style={inp}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            ...
            <div><span style={lbl}>Useful Life (yrs)</span><input type="number" value={formData.assetLife || USEFUL_LIFE[formData.assetCategory as keyof typeof USEFUL_LIFE] || 5} onChange={e => set({ assetLife: e.target.value })} style={inp} /></div>
          </div>
        </>}
