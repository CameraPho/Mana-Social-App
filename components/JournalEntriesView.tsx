...
export default function JournalEntriesView(p: any) {
  ...
  const reverseEntry = async (entry: any) => {
    ...
    try {
      const { data: revJE, error: jeErr } = await supabase
        .from('journal_entries')
        .insert({
          entry_date: today(),
      ...
      await supabase
        .from('journal_entries')
        .update({ is_reversed: true, reversed_by_id: revJE.id })
        .eq('id', entry.id)

      fetchData()
    } catch (err: any) {
      alert('Reverse error: ' + err.message)
    }
  }

  const filteredEntries = useMemo(() => {
    if (filterSource === 'all') return entries
    return entries.filter(e => e.source_type === filterSource)
  ...
  if (loading) return (
    <div style={{ ...card, padding: '40px', textAlign: 'center', color: C.muted }}>Loading journal entries...</div>
  )
    ...
    <div>
      ...
      {!showNewForm && (
        <button
          ...
          style={{ width: '100%', padding: '12px', borderRadius: '10px', border: `1px solid ${C.teal}`, background: 'rgba(45,191,184,0.08)', fontSize: '14px', fontWeight: 'bold', color: C.teal, cursor: 'pointer', fontFamily: FONT, marginBottom: '12px' }}
        >
          + New Journal Entry
        </button>
      )}
        ...
        <div style={{ ...card, border: `1px solid ${C.teal}` }}>
          ...
          <div style={{ display: 'grid', gap: '10px', marginBottom: '14px' }}>
            <div>
              ...
              <input type="date" value={newEntry.entry_date} onChange={e => setNewEntry({ ...newEntry, entry_date: e.target.value })} style={inp} />
            </div>
            <div>
              <span style={lbl}>Description</span>
              <input value={newEntry.description} onChange={e => setNewEntry({ ...newEntry, description: e.target.value })} placeholder="e.g. Owner contribution from Cam" style={inp} />
            </div>
            <div>
              <span style={lbl}>Notes (optional)</span>
              <input value={newEntry.notes} onChange={e => setNewEntry({ ...newEntry, notes: e.target.value })} placeholder="Memo, reference number, etc." style={inp} />
            </div>
          </div>
          ...
          {newLines.map((line, idx) => (
            <div key={idx} style={{ background: C.inputBg, padding: '10px', borderRadius: '8px', marginBottom: '6px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                ...
                {newLines.length > 2 && (
                  <button onClick={() => removeLine(idx)} style={{ ...editBtn, color: '#ef4444', borderColor: '#ef444440' }}>Remove</button>
                )}
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px', marginBottom: '6px' }}>
                <div>
                  <span style={{ fontSize: '10px', color: C.muted }}>Debit</span>
                  <input type="number" step="0.01" placeholder="0.00" value={line.debit} onChange={e => updateLine(idx, 'debit', e.target.value)} style={{ ...inp, fontSize: '12px' }} />
                </div>
                <div>
                  <span style={{ fontSize: '10px', color: C.muted }}>Credit</span>
                  <input type="number" step="0.01" placeholder="0.00" value={line.credit} onChange={e => updateLine(idx, 'credit', e.target.value)} style={{ ...inp, fontSize: '12px' }} />
                </div>
              </div>
          ...
          <div style={{ marginTop: '12px', padding: '10px', background: isBalanced ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.08)', borderRadius: '8px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
              <span style={{ color: C.muted, fontWeight: 'bold' }}>Total Debit:</span>
              <span style={{ fontWeight: 700, color: C.text }}>{fmt(totalDebit)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', marginTop: '4px' }}>
              <span style={{ color: C.muted, fontWeight: 'bold' }}>Total Credit:</span>
              <span style={{ fontWeight: 700, color: C.text }}>{fmt(totalCredit)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', marginTop: '6px', paddingTop: '6px', borderTop: `1px solid ${C.border}` }}>
              <span style={{ color: isBalanced ? '#10B981' : '#ef4444', fontWeight: 'bold' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
            <button onClick={saveEntry} disabled={!isBalanced} style={{ flex: 1, padding: '12px', background: isBalanced ? `linear-gradient(135deg,${C.teal},#1A7A75)` : C.muted, color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: isBalanced ? 'pointer' : 'not-allowed', fontFamily: FONT }}>
              Post Entry
            </button>
            <button onClick={resetForm} style={{ padding: '12px 16px', background: 'none', border: `1px solid ${C.border}`, borderRadius: '8px', color: C.muted, cursor: 'pointer', fontFamily: FONT }}>
              Cancel
            </button>
          </div>
        </div>
      ...
      {sourceTypes.length > 0 && (
        <div style={{ marginBottom: '10px' }}>
          ...
          <select value={filterSource} onChange={e => setFilterSource(e.target.value)} style={inp}>
            ...
            {sourceTypes.map(s => {
              const count = entries.filter(e => e.source_type === s).length
      ...
      {filteredEntries.length === 0 ? (
        <div style={{ ...card, padding: '40px', textAlign: 'center' }}>
          <div style={{ fontSize: '14px', color: C.muted }}>No journal entries yet.</div>
          <div style={{ fontSize: '12px', color: C.muted, marginTop: '4px' }}>Create one above, or wait for Stage 9.3 backfill.</div>
        </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '11px', color: C.muted, marginTop: '2px' }}>
                    {entry.entry_date} · {(entry.journal_entry_lines || []).length} lines
                  </div>
                </div>
                ...
                <div style={{ marginTop: '10px' }}>
                  {(entry.journal_entry_lines || []).sort((a: any, b: any) => (a.line_order || 0) - (b.line_order || 0)).map((line: any) => (
                    <div key={line.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: '8px', padding: '6px 0', borderBottom: `1px solid ${C.border}`, fontSize: '12px' }}>
                      ...
                      <div style={{ fontFamily: 'monospace', textAlign: 'right', minWidth: '80px' }}>
                        {Number(line.debit) > 0 ? <span style={{ fontWeight: 700, color: C.text }}>{fmt(Number(line.debit))}</span> : <span style={{ color: C.muted }}>—</span>}
                      </div>
                      <div style={{ fontFamily: 'monospace', textAlign: 'right', minWidth: '80px' }}>
                        {Number(line.credit) > 0 ? <span style={{ fontWeight: 700, color: C.text }}>{fmt(Number(line.credit))}</span> : <span style={{ color: C.muted }}>—</span>}
                      </div>
                    </div>
                  ))}
                </div>
